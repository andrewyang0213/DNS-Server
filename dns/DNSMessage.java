...
public class DNSMessage {
    ...
    // track the next byte to read from the buffer when building a request
    // message
    ...
    // note: only supports a single question per packet
    ...
    public DNSMessage(DatagramPacket pkt) {
        ...
        // track current position in the UDP packet byte buffer
        next_byte = 0;
    ...
    /**
     * constructor to make a DNS Message response object given a DNS request message
     * and record information
     *
     * @param request         a complete DNS request message
     * @param answers         the list of answers to includ
     * @param isAuthoritative whether this answer is authoritative (came from zone)
     *                        or not (came from the cache or another server)
     */
    ...
    /**
     * utility method to initialize the message fields from ID through the
     * question/record numbers
     *
     * @param request         a complete DNS request message
     * @param isAuthoritative whether this answer is authoritative (came from zone)
     *                        or not (came from the cache or another server)
    ...
    private void createHeader(DNSMessage request, boolean isAuthoritative) {
        // the id, opcode flag, recursion desired flag, and #questions is the same as in
        // the request message
        this.id = request.id;
        ...
        // set the recursion available flag to 1
        this.flag_ra = 1;
        ...
        if (answers.size() != 0) {
            // found a record -> response code is 0 with the given number of answers
            this.flag_rcode = 0;
            this.num_answers = answers.size();
        } else {
            // didn't find a record -> response code is 3 with 0 answers
            this.flag_rcode = 3;
            this.num_answers = 0;
        }
    ...
    /**
     * utility method to fill in the byte buffer from the instance variables in this
     * object
     */
    private void createBuffer() {
        ...
        // track how many bytes added to the buffer
    ...
    private void parseHeader() {
        ...
        // the first 12 bytes in the message are the 6 2-byte fields that start the
        // message
    ...
    private String parseCompressedName() {
        ...
        // save location parsing through the message
    ...
    private String parseName() {
        ...
        // otherwise, the name is stored as a series of one-byte lengths followed by
        // that many chars, each representing a single label
    ...
    private void parseQuestions() {
        // only support a single question
        if (num_questions != 1) {
    ...
    public String toString() {
        ...
        if (num_answers != 0) {
            ...
            for (var answer : answers) {
                sb.append(String.format("- %s, %s, %s, %d, %s%n", answer.getName(), answer.getTypeStr(),
                        answer.getClassStr(), answer.getTTL(), answer.getData()));
            }
        }
     ...
     *
     * @return the number of bytes in the full byte buffer representation of this
     *         message
     */
